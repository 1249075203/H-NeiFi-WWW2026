...
m = 1       # common neighbor
beta = 0
p = 0.5     # pressure level
lam = 0.5   # suffer pressure %
...
for episode in range(50):
    ...
    while True: # [1, test_times]
        ...
        for i in range(args.agent_num):
            ...
            # print("neighbour_value/neighbour_num", neighbour_value/neighbour_num)
            if i in N_p:
                # next_agent_value[i] = (1-p) * neighbour_value/neighbour_num + p * avg_agent_value
                temp_next = (1-p) * neighbour_value/neighbour_num + p * avg_agent_value
                temp_next = args.weight_exp * agent_exp_opinion + args.weight_normal * temp_next
                next_agent_value[i] = agent_confident[i]* agent_value[i] + (1-agent_confident[i])*temp_next
                FLOPs += 4
            else:
                # next_agent_value[i] = neighbour_value/neighbour_num
                temp_next = neighbour_value/neighbour_num
                temp_next = args.weight_exp * agent_exp_opinion + args.weight_normal * temp_next
                next_agent_value[i] = agent_confident[i] * agent_value[i] + (1 - agent_confident[i]) * temp_next
                FLOPs += 1

            # print(next_agent_value)
            # sleep(5)
    ...
    plt.savefig(f'result_GP/{args.agent_num}-{args.opinions_end}-{episode}.jpg', dpi=600, bbox_inches='tight', format='jpg')
    # episode = 1
    # plt.savefig(f'C:\\Users\\Desktop\\P\\CNR/eps/fig_agent_{episode}.eps',dpi=600,bbox_inches='tight',format='eps')
