...
for episode in range(50):
    ...
    while True: # [1, test_times]
        ...
        if is_stable(agent_value):
            ...
            print('CLUSTERS:', cu_num)
            cal_miss(agent_value, Args.target_opinions)
            miss_values = cal_miss(agent_value, Args.target_opinions)
            print("RES:", abs(miss_values))
            print("STEPS", test_times)
            print("BREAK_STEPS",test_times)
            break
    ...
    # plt.xticks(np.arange(0, test_times+1, step=5))
    # plt.yticks(np.arange(0, interval+1, step=5))
    # plt.title(f'discrete-time system {num} agents uniformly spaced')
